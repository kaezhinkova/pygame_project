...
class Cat(pygame.sprite.Sprite):
    ...
    def current_block(self, rect_list):
        for i in range(len(rect_list)):
            if rect_list[i].colliderect([self.player_x + 10, self.player_y, 50, 40]):
                return i
            if rect_list[i].colliderect([self.player_x - 10, self.player_y, 50, 40]):
                return i
    ...
    def check_collisions(self, rect_list, j):
        for i in range(len(rect_list)):
            if rect_list[i].colliderect([self.player_x, self.player_y, 50, 40]) and rect_list[self.curr_block][0] < self.player_x and i != self.curr_block:# platform left
                self.curr_block = i
                return True

            if rect_list[i].colliderect([self.player_x, self.player_y, 50, 40]) and rect_list[self.curr_block][0] > self.player_x and i != self.curr_block:
                self.curr_block = i
                return True
...
while running:
    ...
    '''
        ...
        if blocks[curr_block][0] <= cat.player_x:
    ...
    if cat.check_fall(cat.player_x, cat.player_y):
        cat.jump = False
    if cat.jump_1 and cat.jump and not cat.check_collisions(blocks, cat.curr_block):
